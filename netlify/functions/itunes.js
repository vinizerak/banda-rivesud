...
:root {
  ...
  --accent-mid: rgba(255,255,255,0.7);
  --tab-h: 44px;
  --album-c1: #1a1a2e;
  --album-c2: #534AB7;
  --album-text: #ffffff;
...
body::before {
  ...
  position: fixed;
  inset: 0;
  background: var(--cf-bg-full, #0d0d0d);
  transition: background 0.7s ease;
  z-index: -2;
  pointer-events: none;
}
body::after {
  ...
  position: fixed;
  inset: 0;
  background-image: var(--cf-cover-url, none);
...
.other-nav-pill {
  ...
  border-radius: 99px; color: rgba(255,255,255,0.6);
...
/* ── MUDANÇA 5: nav sempre visível no topo ── */
#cf-nav-overlay {
  position: absolute;
...
/* removido: transform translateY(-100%) e transition — sempre visível */
...
.cf-nav-pill:hover:not(.active) { background: rgba(255,255,255,0.12); color: rgba(255,255,255,0.85); }
...
.cf-search-input {
  ...
  border-radius: 10px; font-size: 14px; font-family: 'DM Sans', sans-serif; color: #fff; outline: none;
}
...
.cf-filter-btn {
  ...
  color: rgba(255,255,255,0.55); cursor: pointer; white-space: nowrap; transition: all 0.15s;
}
.cf-filter-btn.on { background: rgba(255,255,255,0.2); border-color: rgba(255,255,255,0.4); color: #fff; font-weight: 500; }

/* nav height agora é ~100px — count badge abaixo dela */
#cf-count-badge { position: absolute; top: 108px; right: 16px; z-index: 30; font-size: 11px; color: rgba(255,255,255,0.25); }

/* cards: top 50% deslocado para baixo da nav */
#cf-cards {
  position: absolute;
...
/* ── PLAYER STRIP músicas: entre foto e nome ── */
#cf-player-strip {
  position: absolute;
  left: 0; right: 0; z-index: 21;
  /* posicionado entre cards e nome via bottom */
  bottom: 130px;
...
.cfps-mute:active { background: rgba(255,255,255,0.1); }
.cfps-mute svg { width: 22px; height: 22px; fill: currentColor; display: block; }
.cfps-play-btn {
...
.cfps-play-btn:active { background: rgba(255,255,255,0.22); }
.cfps-play-btn svg { width: 15px; height: 15px; fill: #fff; display: block; }

#cf-song-info { position: absolute; bottom: 90px; left: 0; right: 0; text-align: center; z-index: 20; padding: 0 24px; pointer-events: none; }
#cf-song-name { font-size: clamp(20px,5vw,28px); font-weight: 600; color: #fff; text-shadow: 0 2px 12px rgba(0,0,0,0.5); line-height: 1.2; }
#cf-song-artist { font-size: clamp(13px,3vw,16px); color: rgba(255,255,255,0.55); margin-top: 4px; }

#cf-dots { position: absolute; bottom: 68px; left: 0; right: 0; display: flex; justify-content: center; gap: 5px; z-index: 20; flex-wrap: wrap; max-width: 200px; margin: 0 auto; }
.cf-dot { width: 5px; height: 5px; border-radius: 50%; background: rgba(255,255,255,0.3); transition: all 0.3s; cursor: pointer; }
.cf-dot.active { background: #fff; width: 14px; border-radius: 3px; }

/* audio original — ainda usado para preview iTunes na aba músicas */
#cf-audio-bar { position: absolute; bottom: 16px; left: 20px; right: 20px; display: none; align-items: center; gap: 10px; padding: 8px 14px; background: rgba(0,0,0,0.4); backdrop-filter: blur(12px); border: 0.5px solid rgba(255,255,255,0.12); border-radius: 10px; z-index: 20; }
#cf-audio-icon { font-size: 14px; flex-shrink: 0; }
#cf-audio-title { font-size: 12px; color: rgba(255,255,255,0.6); flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
#cf-audio-status { font-size: 11px; color: rgba(255,255,255,0.7); flex-shrink: 0; }
#cf-audio { display: none; }
...
.c-seg { flex: 1; max-width: 96px; text-align: center; padding: 6px 6px; background: rgba(0,0,0,0.32); backdrop-filter: blur(14px); border: 0.5px solid rgba(255,255,255,0.09); border-right: none; }
...
.c-sv { font-size: 16px; font-weight: 700; line-height: 1; color: #fff; }
.c-sl { font-size: 9px; color: rgba(255,255,255,0.38); margin-top: 2px; text-transform: uppercase; letter-spacing: 0.05em; }

#c-months { position: fixed; top: 108px; left: 0; right: 0; z-index: 110; display: none; gap: 6px; padding: 4px 14px; overflow-x: auto; scrollbar-width: none; justify-content: center; }
#c-months.vis { display: flex; }
#c-months::-webkit-scrollbar { display: none; }
.c-mpill { flex-shrink: 0; height: 27px; padding: 0 12px; background: rgba(255,255,255,0.07); border: 0.5px solid rgba(255,255,255,0.12); border-radius: 99px; font-size: 11px; font-weight: 500; color: rgba(255,255,255,0.45); cursor: pointer; transition: all 0.22s; white-space: nowrap; }
.c-mpill.active { background: rgba(255,255,255,0.2); color: #fff; border-color: rgba(255,255,255,0.38); }

/* ── MUDANÇA 2: pos-l com translateX menor para não cortar ── */
#c-stage { position: fixed; left: 0; right: 0; z-index: 20; top: 146px; height: min(162px,44vw); display: none; align-items: center; justify-content: center; overflow: visible; }
#c-stage.vis { display: flex; }
#c-cards { position: relative; width: 100%; height: 100%; overflow: visible; }
.cc { position: absolute; top: 0; left: 50%; margin-left: calc(min(240px,68vw) / -2); width: min(240px,68vw); height: min(162px,44vw); border-radius: 20px; overflow: hidden; cursor: pointer; transition: transform 0.42s cubic-bezier(0.25,0.46,0.45,0.94), filter 0.42s ease, opacity 0.42s ease; background: rgba(255,255,255,0.08); border: 0.5px solid rgba(255,255,255,0.18); backdrop-filter: blur(24px); }
.cc.pos-c { transform: translateX(0) scale(1); filter: brightness(1); z-index: 10; box-shadow: 0 20px 50px rgba(0,0,0,0.6); }
/* CORRIGIDO: era min(-190px,-52vw) — causava corte. Agora -155px fixo */
.cc.pos-l { transform: translateX(-155px) scale(0.82); filter: brightness(0.45); z-index: 5; }
.cc.pos-r { transform: translateX(155px) scale(0.82); filter: brightness(0.45); z-index: 5; }
.cc.pos-hl { transform: translateX(-340px) scale(0.6); opacity: 0; z-index: 1; pointer-events: none; }
.cc.pos-hr { transform: translateX(340px) scale(0.6); opacity: 0; z-index: 1; pointer-events: none; }
...
.cc-body { width: 100%; height: 100%; padding: 16px 18px; display: flex; flex-direction: column; justify-content: space-between; }
.cc-top { display: flex; align-items: flex-start; gap: 9px; }
.cc-day { font-size: 46px; font-weight: 800; line-height: 0.9; letter-spacing: -0.03em; color: rgba(255,255,255,0.95); }
.cc-meta { display: flex; flex-direction: column; padding-top: 5px; }
.cc-dow { font-size: 9px; color: rgba(255,255,255,0.45); text-transform: uppercase; letter-spacing: 0.07em; }
.cc-myr { font-size: 11px; font-weight: 500; color: rgba(255,255,255,0.65); margin-top: 2px; }
.cc-leader { font-size: 13px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.cc-count { font-size: 9px; color: rgba(255,255,255,0.38); margin-top: 2px; text-transform: uppercase; letter-spacing: 0.05em; }

#c-info { position: fixed; left: 0; right: 0; z-index: 20; top: calc(146px + min(162px,44vw) + 8px); display: none; flex-direction: column; align-items: center; gap: 5px; }
...
/* ── MUDANÇA 4+6: song stage vira lista ── */
#c-song-stage {
  position: fixed; left: 0; right: 0; z-index: 20;
  top: calc(146px + min(162px,44vw) + 54px);
  bottom: 0;
...
/* lista de músicas */
#c-song-list { width: 100%; max-width: 520px; display: flex; flex-direction: column; gap: 5px; flex: 1; min-height: 0; }

.csl-item {
  background: rgba(0,0,0,0.38); backdrop-filter: blur(18px);
...
.csl-item:active { background: rgba(255,255,255,0.06); }
.csl-item.active { background: rgba(0,0,0,0.55); border-color: rgba(255,255,255,0.22); padding: 12px 15px; flex: 1.7; }
.csl-row { display: flex; align-items: center; gap: 9px; }
.csl-num { font-size: 10px; font-weight: 700; color: rgba(255,255,255,0.2); width: 16px; flex-shrink: 0; text-align: right; }
.csl-item.active .csl-num { color: rgba(255,255,255,0.45); }
...
.csl-tom { font-size: 10px; font-weight: 700; background: rgba(255,255,255,0.1); border: 0.5px solid rgba(255,255,255,0.2); border-radius: 6px; padding: 2px 7px; color: rgba(255,255,255,0.55); flex-shrink: 0; }
.csl-item.active .csl-tom { color: #fff; background: rgba(255,255,255,0.16); border-color: rgba(255,255,255,0.32); }
.csl-expanded { max-height: 0; overflow: hidden; opacity: 0; transition: max-height 0.28s cubic-bezier(0.25,0.46,0.45,0.94), opacity 0.22s; }
.csl-item.active .csl-expanded { max-height: 50px; opacity: 1; }
.csl-artist { font-size: 11px; color: rgba(255,255,255,0.35); margin-top: 4px; padding-left: 25px; }
...
.cpb-controls { display: flex; align-items: center; gap: 8px; }
.cpb-btn { background: none; border: none; padding: 5px; cursor: pointer; color: rgba(255,255,255,0.6); display: flex; align-items: center; justify-content: center; border-radius: 50%; transition: background 0.15s; flex-shrink: 0; }
.cpb-btn:active { background: rgba(255,255,255,0.1); }
.cpb-btn svg { width: 18px; height: 18px; fill: currentColor; display: block; }
.cpb-play { width: 34px; height: 34px; border-radius: 50%; background: rgba(255,255,255,0.14); border: 0.5px solid rgba(255,255,255,0.26); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: background 0.15s; flex-shrink: 0; }
.cpb-play:active { background: rgba(255,255,255,0.26); }
.cpb-play svg { width: 12px; height: 12px; fill: #fff; display: block; }
/* scrubber */
.cpb-scrubber { display: flex; flex-direction: column; gap: 3px; }
.cpb-track { position: relative; height: 18px; display: flex; align-items: center; cursor: pointer; }
.cpb-bar { width: 100%; height: 3px; background: rgba(255,255,255,0.16); border-radius: 99px; pointer-events: none; }
.cpb-fill { height: 100%; background: rgba(255,255,255,0.75); border-radius: 99px; position: relative; transition: width 0.4s linear; pointer-events: none; }
.cpb-thumb { position: absolute; right: -5px; top: 50%; transform: translateY(-50%); width: 11px; height: 11px; border-radius: 50%; background: #fff; pointer-events: none; }
.cpb-times { display: flex; justify-content: space-between; font-size: 9px; color: rgba(255,255,255,0.3); }

/* FAB cultos */
#c-fab { position: fixed; bottom: 24px; right: 20px; z-index: 115; width: 50px; height: 50px; border-radius: 50%; background: #fff; color: #0d0d0d; font-size: 26px; font-weight: 300; border: none; cursor: pointer; display: none; align-items: center; justify-content: center; box-shadow: 0 4px 20px rgba(0,0,0,0.55); transition: transform 0.25s; }
#c-fab.vis { display: flex; }
#c-fab:hover { transform: scale(1.06); }
...
/* ── MUDANÇA 8: ensaio padding-top maior ── */
#panel-ensaio { display: none; padding-top: 76px; }
#panel-ensaio.active { display: block; }
.ensaio-choice-row { display: flex; gap: 12px; margin-bottom: 20px; padding: 0 16px; }
...
.ensaio-choice-card:hover { border-color: var(--accent-mid); background: var(--accent-light); }
...
.ensaio-culto-info { font-size: 12px; color: var(--muted); margin-bottom: 12px; padding: 0 18px; }
...
.ensaio-song-btn:hover { border-color: var(--accent-mid); background: var(--accent-light); }
.ensaio-song-btn:active { transform: scale(0.99); }
...
.ensaio-ext-link { font-size: 13px; color: rgba(255,255,255,0.6); background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); padding: 8px 18px; border-radius: 99px; text-decoration: none; margin-top: 4px; transition: all 0.15s; }
.ensaio-ext-link:hover { background: rgba(255,255,255,0.14); color: #fff; }
#ensaio-bar { position: fixed; bottom: 0; left: 0; right: 0; height: 56px; background: rgba(10,20,6,0.96); backdrop-filter: blur(12px); border-top: 1px solid rgba(255,255,255,0.08); display: none; align-items: center; z-index: 510; user-select: none; }
#ensaio-bar.open { display: flex; }
.ensaio-bar-btn { flex-shrink: 0; height: 56px; width: 64px; background: none; border: none; color: #fff; font-size: 24px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background 0.12s; -webkit-tap-highlight-color: transparent; }
...
.ensaio-error-vini { width: 100%; max-width: 440px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 14px 18px; font-size: 13px; color: rgba(255,255,255,0.35); line-height: 1.5; }
.ensaio-error-back { margin-top: 4px; padding: 13px 32px; background: rgba(93,187,58,0.15); border: 1px solid rgba(93,187,58,0.3); color: #5DBB3A; border-radius: 99px; font-size: 15px; font-family: 'DM Sans', sans-serif; cursor: pointer; transition: all 0.15s; }
.ensaio-error-back:hover { background: rgba(93,187,58,0.25); }
...
.ensaio-menu-item:hover { background: rgba(255,255,255,0.06); }
...
@media (min-width: 600px) {
  .ensaio-btn-nav { width: 64px; height: 96px; font-size: 30px; }
  .ensaio-btn-close, .ensaio-btn-menu { width: 50px; height: 50px; font-size: 20px; }
}
</style>
</head>
<body>
...
<div id="panel-repertorio" class="panel active">
  ...
  <!-- nav sempre visível (sem toggle) -->
  <div id="cf-nav-overlay">
  ...
  <!-- player strip: mute + play, entre foto e nome -->
  <div id="cf-player-strip" class="hidden">
    <button class="cfps-mute" id="cfps-mute-btn" onclick="gMuteToggle()">
    ...
    <div class="cfps-play-btn" onclick="gPlayPause()">
  ...
  <div id="cf-audio-bar">
    <span id="cf-audio-icon">&#127925;</span>
    <span id="cf-audio-title">Preview</span>
    <span id="cf-audio-status"></span>
  </div>
  <audio id="cf-audio" preload="none"></audio>

  <!-- sheet detalhes música -->
  <div id="cf-sheet">
...
<div id="panel-cultos" class="panel">
  <div id="c-bg"></div>
  <div id="c-bg-blur"></div>
  <div id="c-bg-overlay"></div>
</div>
...
<div id="panel-ensaio" class="panel">
  <div class="ensaio-choice-row">
    <button class="ensaio-choice-card" id="choice-instrumentista" onclick="selecionarPerfil('instrumentista')">
      <span class="ensaio-choice-icon">🎸</span>
      <span class="ensaio-choice-lbl">Instrumentista</span>
      <span class="ensaio-choice-sub">Cifras do culto</span>
    </button>
    <button class="ensaio-choice-card" id="choice-vocalista" onclick="selecionarPerfil('vocalista')">
      <span class="ensaio-choice-icon">🎤</span>
      <span class="ensaio-choice-lbl">Vocalista</span>
      <span class="ensaio-choice-sub">Letras ao vivo</span>
    </button>
  </div>
...
<div id="c-stats">
  ...
  <div class="c-seg btn" onclick="cAbrirTopMes()"><div class="c-sv" id="c-sv-top" style="font-size:11px;line-height:1.3;text-align:center;">—</div><div class="c-sl">top mês ›</div></div>
</div>
<div id="c-months"></div>
<div id="c-stage"><div id="c-cards"></div></div>
<div id="c-info"><div id="c-pos"></div><div id="c-dots"></div></div>

<!-- song stage: nova lista -->
<div id="c-song-stage">
  <div id="c-song-list"></div>
  <div id="c-player-bar">
    <div class="cpb-top">
      <div class="cpb-info">
        <div class="cpb-name" id="cpb-name">—</div>
        <div class="cpb-artist" id="cpb-artist"></div>
      </div>
      <div class="cpb-controls">
        <button class="cpb-btn" id="cpb-mute-btn" onclick="gMuteToggle()">
          <svg id="cpb-mute-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
        </button>
        <div class="cpb-play" id="cpb-play-btn" onclick="gPlayPause()">
          <svg id="cpb-play-icon" viewBox="0 0 24 24" fill="#fff"><path d="M8 5v14l11-7z"/></svg>
        </div>
      </div>
    </div>
    <div class="cpb-scrubber">
      <div class="cpb-track" id="cpb-track">
        <div class="cpb-bar"><div class="cpb-fill" id="cpb-fill" style="width:0%"><div class="cpb-thumb"></div></div></div>
      </div>
...
<div id="c-tm-ov" onclick="if(event.target===this)cFecharTopMes()">
  <div id="c-tm-sh">
    <div class="c-tm-hand"></div>
    <div class="c-tm-lbl">Este mês</div>
    <div class="c-tm-title" id="c-tm-title">—</div>
    <div class="c-tm-sub" id="c-tm-sub"></div>
...
<div id="holyrics-fs">
  ...
  <div class="holyrics-bar">
    <button onclick="fecharHolyrics()" style="background:none;border:none;color:rgba(255,255,255,.5);font-size:14px;cursor:pointer;font-family:'DM Sans',sans-serif;padding:8px 0;">✕ Fechar</button>
...
<div id="ensaio-menu" onclick="if(event.target===this)fecharMenuEnsaio()">
  <div class="ensaio-menu-sheet">
    <div class="ensaio-menu-handle"></div>
    <div class="ensaio-menu-title">Setlist</div>
...
<div id="ensaio-error">
  ...
  <div class="ensaio-error-desc">Não foi possível carregar o ensaio. Manda esse erro pro Vinicius 👇</div>
  <div class="ensaio-error-box" id="ensaio-error-box"></div>
  <div class="ensaio-error-vini">📲 Vinicius, copia o erro acima e me manda!</div>
...
const gPlayer = {
  ...
  play(url, nome, artista) {
    // para qualquer coisa que esteja tocando
    ...
    this.audio.addEventListener('loadedmetadata', () => this._tick());
  ...
  pause() { if (this.audio) this.audio.pause(); this._updateIcons(false); },
  resume() { if (this.audio) { this.audio.play(); this._updateIcons(true); } },
  ...
  _tick() {
    ...
    this._setProgress(pct, cur, dur);
    this._updateIcons(!this.audio.paused);
  },

  _setProgress(pct, cur, dur) {
    ...
    const cpbCur = document.getElementById('cpb-cur');
    if (cpbCur) cpbCur.textContent = fmt(cur);
    const cpbDur = document.getElementById('cpb-dur');
    if (cpbDur) cpbDur.textContent = fmt(dur);
  },

  _showBars() {
    // mostra player bar cultos
    const bar = document.getElementById('c-player-bar');
    if (bar) { bar.classList.add('vis'); }
    const nameEl = document.getElementById('cpb-name');
    if (nameEl) nameEl.textContent = this.nome;
    const artistEl = document.getElementById('cpb-artist');
    if (artistEl) artistEl.textContent = this.artista;
    // mostra strip músicas
    const strip = document.getElementById('cf-player-strip');
    if (strip) strip.classList.remove('hidden');
    this._updateMuteIcons();
  },

  _updateIcons(playing) {
    const pauseSVG = '<path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>';
    const playSVG  = '<path d="M8 5v14l11-7z"/>';
    const svg = playing ? pauseSVG : playSVG;
    const cfIcon = document.getElementById('cfps-play-icon');
    if (cfIcon) cfIcon.innerHTML = svg;
    const cpbIcon = document.getElementById('cpb-play-icon');
    if (cpbIcon) cpbIcon.innerHTML = svg;
  },

  _updateMuteIcons() {
    const onSVG  = '<path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/>';
    const offSVG = '<path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/>';
    const svg = this.muted ? offSVG : onSVG;
    const cfMute = document.getElementById('cfps-mute-icon');
    if (cfMute) cfMute.innerHTML = svg;
    const cpbMute = document.getElementById('cpb-mute-icon');
    if (cpbMute) cpbMute.innerHTML = svg;
  }
};
...
// seek no scrubber cultos
document.addEventListener('DOMContentLoaded', () => {
  const track = document.getElementById('cpb-track');
  if (track) {
    track.addEventListener('click', e => {
      const r = track.getBoundingClientRect();
      gPlayer.seekTo((e.clientX - r.left) / r.width);
    });
  }
  // sheet backdrop
  ...
  // drag handle sheet
  let sheetDragY = null;
  document.getElementById('cf-sheet-drag').addEventListener('touchstart', e => { sheetDragY = e.touches[0].clientY; }, {passive:true});
  document.getElementById('cf-sheet-drag').addEventListener('touchend', e => {
    if (sheetDragY === null) return;
    if (e.changedTouches[0].clientY - sheetDragY > 60) cfFecharSheet();
    sheetDragY = null;
  }, {passive:true});
  // touch nav músicas
  ...
  let msx = null;
  panel.addEventListener('mousedown', e => { msx = e.clientX; });
  panel.addEventListener('mouseup', e => {
    if (msx===null || document.getElementById('cf-sheet').classList.contains('open')) return;
    const dx = e.clientX - msx;
    if (Math.abs(dx)>44) cfNavegar(dx<0?1:-1);
    msx = null;
  });
});
...
async function carregarDados() {
  try {
    ...
    musicas = parseCSVMusicas(m);
    cultos  = parseCSVCultos(c);
    renderMusicas();
    renderCultos();
  } catch(e) {
    toast('Erro ao carregar planilha.', true);
  }
}
...
function parseCSVLine(ln) {
  ...
  for (let i=0;i<ln.length;i++) {
    ...
    if (ch==='"') { if(inQ&&nx==='"'){v+='"';i++;} else inQ=!inQ; }
    else if (ch===','&&!inQ) { C.push(v); v=''; }
    else v+=ch;
  }
  C.push(v);
...
// MUDANÇA 1: corrigir ano de 2 dígitos
...
// ═══════════════════════════════════════
// COVERFLOW — preservado do original
// ═══════════════════════════════════════
let cfLista=[], cfIdx=0, cfCovers={};
let cfTouchStartX=0, cfTouchStartY=0, cfTouchStartTime=0;
...
const CF_GRADIENTS = [
  ...
  ['#0a1a2a','#4A7C28'],['#2a1a0a','#BA7517'],['#1a2a1a','#3B6D11'],
];
function cfGradient(nome) { let h=0; for(let i=0;i<nome.length;i++) h=(h*31+nome.charCodeAt(i))&0xffffffff; return CF_GRADIENTS[Math.abs(h)%CF_GRADIENTS.length]; }
function cfInitials(nome) { return nome.split(' ').slice(0,2).map(w=>w[0]||'').join('').toUpperCase()||'♪'; }

async function cfBuscarCapa(musica) {
  const key = musica.nome+'|'+musica.artista;
  if (cfCovers[key] !== undefined) return cfCovers[key];
  cfCovers[key] = null;
  const q = (musica.nome+' '+musica.artista).trim();
  try {
    const r = await Promise.race([
      fetch(`/.netlify/functions/itunes?q=${encodeURIComponent(q)}`),
      new Promise((_,rej) => setTimeout(()=>rej(),5000))
    ]);
    const d = await r.json();
    if (d.results?.length > 0) {
      const item = d.results[0];
      cfCovers[key] = { img:(item.artworkUrl100||'').replace('100x100bb','400x400bb'), preview:item.previewUrl||null, color:d.dominantColor||null };
      return cfCovers[key];
    }
  } catch(e) {}
  cfCovers[key] = { img:null, preview:null, color:null };
  return cfCovers[key];
}
...
function renderMusicas() {
  ...
  if (ordenarPorRecente) {
    lista.forEach(m => {
      let ud=null;
      cultos.forEach(c=>{if(c.musicas.some(cm=>cm.nome.toLowerCase()===m.nome.toLowerCase())){const d=parseDataBR(c.data);if(!ud||d>ud)ud=d;}});
      m.ultimaData=ud;
    });
    ...
    lista.sort((a,b)=>b.usos-a.usos||a.nome.localeCompare(b.nome));
  }
  ...
  if (!lista.length) {
    ...
    document.getElementById('cf-song-name').textContent='Nenhuma música encontrada';
    document.getElementById('cf-song-artist').textContent='';
    return;
  }
  cfRenderCarrossel();
  cfPreloadCovers();
...
function cfRenderCarrossel(dir) {
  if (!cfLista.length) return;
  const total = cfLista.length;
  const container = document.getElementById('cf-cards');
  container.innerHTML = '';
  [-2,-1,0,1,2].forEach(offset => {
    const idx=(cfIdx+offset+total)%total, m=cfLista[idx];
    const grad=cfGradient(m.nome), cover=cfCovers[m.nome+'|'+m.artista];
    const posCls=offset===0?'pos-center':offset===-1?'pos-left':offset===1?'pos-right':offset<-1?'pos-hidden-left':'pos-hidden-right';
    const sizeCls=offset===0?'cf-card-size':'cf-card-size-side';
    let animCls='';
    if (dir!==undefined && offset===0) animCls=dir>0?'anim-enter-right':'anim-enter-left';
    const div=document.createElement('div');
    div.className=`cf-card ${sizeCls} ${posCls} ${animCls}`;
    div.innerHTML=`<div class="cf-cover-fallback" style="background:linear-gradient(135deg,${grad[0]},${grad[1]}88);">${cfInitials(m.nome)}</div>${cover&&cover.img?`<div class="cf-cover-img" style="background-image:url('${cover.img}');"></div>`:''}${offset===0?`<div class="cf-overlay"></div><div class="cf-plays-badge">${m.usos} culto${m.usos!==1?'s':''}</div>`:''}`;
    if (offset===0) div.addEventListener('click',()=>cfAbrirSheet(m));
    else div.addEventListener('click',()=>cfNavegar(offset));
    container.appendChild(div);
  });
  cfAtualizarBg();
  cfAtualizarInfo();
  cfRenderDots();
  cfTocarPreview(); // preview iTunes automático
}

function cfNavegar(dir) {
  if (!cfLista.length) return;
  cfIdx=(cfIdx+dir+cfLista.length)%cfLista.length;
  cfRenderCarrossel(dir);
  setTimeout(()=>cfPreloadCovers(),50);
}

function cfAtualizarBg() {
  if (!cfLista.length) return;
  const m=cfLista[cfIdx], cover=cfCovers[m.nome+'|'+m.artista];
  const bg=document.getElementById('cf-fullbg'), color=document.getElementById('cf-fullbg-color');
  if (cover?.img) { bg.style.backgroundImage=`url('${cover.img}')`; bg.style.opacity='1'; color.style.background='rgba(0,0,0,0.45)'; }
  else { const g=cfGradient(m.nome); bg.style.backgroundImage=''; bg.style.opacity='0'; color.style.background=`linear-gradient(160deg,${g[0]} 0%,${g[1]}55 100%)`; }
}
...
function cfRenderDots() {
  const dots=document.getElementById('cf-dots'), total=cfLista.length;
  dots.innerHTML='';
  if (total<=1) return;
  const maxDots=12, showTotal=Math.min(total,maxDots);
  let start=Math.max(0,cfIdx-Math.floor(showTotal/2)), end=start+showTotal;
  if (end>total){end=total;start=Math.max(0,end-showTotal);}
  for(let i=start;i<end;i++){const d=document.createElement('div');d.className='cf-dot'+(i===cfIdx?' active':'');d.addEventListener('click',()=>{cfIdx=i;cfRenderCarrossel();});dots.appendChild(d);}
}

async function cfPreloadCovers() {
  if (!cfLista.length) return;
  const total=cfLista.length;
  const prioridade=[0,1,-1].map(o=>(cfIdx+o+total)%total);
  await Promise.all(prioridade.map(async i=>{const m=cfLista[i];if(cfCovers[m.nome+'|'+m.artista]===undefined)await cfBuscarCapa(m);}));
  cfAtualizarBg(); cfRenderCarrossel();
  const resto=cfLista.map((_,i)=>i).filter(i=>!prioridade.includes(i)&&cfCovers[cfLista[i].nome+'|'+cfLista[i].artista]===undefined);
  for(let i=0;i<resto.length;i+=6) await Promise.all(resto.slice(i,i+6).map(idx=>cfBuscarCapa(cfLista[idx])));
}

// preview iTunes — preservado do original (usa #cf-audio separado do gPlayer)
async function cfTocarPreview() {
  const audio=document.getElementById('cf-audio');
  const bar=document.getElementById('cf-audio-bar');
  const statusEl=document.getElementById('cf-audio-status');
  const titleEl=document.getElementById('cf-audio-title');
  audio.pause(); audio.src='';
  if (!cfLista.length) return;
  const m=cfLista[cfIdx];
  titleEl.textContent=m.nome;
  const cover=cfCovers[m.nome+'|'+m.artista]||await cfBuscarCapa(m);
  if (cover?.preview && cfLista[cfIdx]===m) {
    bar.style.display='flex'; statusEl.textContent='carregando…';
    audio.src=cover.preview; audio.volume=0.55;
    try { await audio.play(); statusEl.textContent='▶ preview 30s'; }
    catch(e) { statusEl.textContent='toque ▶'; }
    audio.onended=()=>{ statusEl.textContent='—'; };
  } else { bar.style.display='none'; }
}

// MUDANÇA 3: sheet só com botão play Cloudinary
async function cfAbrirSheet(m) {
  const sheet=document.getElementById('cf-sheet'), grad=cfGradient(m.nome), thumb=document.getElementById('cf-sheet-thumb');
  ...
  // apenas botão play Cloudinary
  const actionsEl=document.getElementById('cf-sheet-actions');
  actionsEl.innerHTML='';
  const audioUrl=resolveAudioUrl(m.midia);
  if (audioUrl) {
    const btn=document.createElement('button');
    btn.className='sheet-btn'; btn.innerHTML='&#9654; Tocar';
    btn.addEventListener('click',()=>{
      gPlayer.play(audioUrl,m.nome,m.artista||'');
      cfFecharSheet();
    });
...
function cBuildMonths(cultos) {
  const map={};
  cultos.forEach(c=>{const d=parseDataBR(c.data),k=`${d.getFullYear()}_${d.getMonth()}`;if(!map[k])map[k]={year:d.getFullYear(),month:d.getMonth(),cultos:[]};map[k].cultos.push(c);});
...
function cGetTopMes(mes,ano) {
  const freq={};
  cultos.forEach(c=>{const d=parseDataBR(c.data);if(d.getMonth()!==mes||d.getFullYear()!==ano)return;c.musicas.forEach(m=>{const k=m.nome.toLowerCase();freq[k]=(freq[k]||0)+1;});});
  return Object.entries(freq).sort((a,b)=>b[1]-a[1]).map(([k,cnt])=>({nome:cultos.flatMap(c=>c.musicas).find(m=>m.nome.toLowerCase()===k)?.nome||k,cnt}));
}
function cInitView() {
  if (!cMonths.length) return;
  const now=new Date(),idx=cMonths.findIndex(m=>m.month===now.getMonth()&&m.year===now.getFullYear());
  cMIdx=idx>=0?idx:0; cCIdx=0; cSIdx=0;
  cRenderMonths(); cRenderAll();
}
...
function cUpdateBg() {
  if (!cMonths.length) return;
  const [c1,c2]=C_PALETTES[cMonths[cMIdx].month];
  document.getElementById('c-bg').style.background=`linear-gradient(160deg,${c1} 0%,#0d0d0d 100%)`;
  document.getElementById('c-bg-blur').style.background=`radial-gradient(ellipse at 30% 30%,${c2}88 0%,transparent 70%)`;
}
...
function cRenderCarousel() {
  ...
  [-2,-1,0,1,2].forEach(offset=>{
    if (!total) return;
    const idx=(cCIdx+offset+total)%total, c=cs[idx], d=parseDataBR(c.data);
    const pos=offset===0?'pos-c':offset===-1?'pos-l':offset===1?'pos-r':offset<-1?'pos-hl':'pos-hr';
    ...
    if (offset!==0) div.addEventListener('click',()=>cNavCulto(offset<0?-1:1));
...
// MUDANÇA 4: lista de músicas
function cRenderSongList() {
  const cs=cMonths[cMIdx]?.cultos||[], songs=cs[cCIdx]?.musicas||[];
  const el=document.getElementById('c-song-list');
  el.innerHTML='';
  songs.forEach((m,i)=>{
    const info=musicas.find(x=>x.nome.toLowerCase()===m.nome.toLowerCase());
    const isActive=i===cSIdx;
    const item=document.createElement('div');
    item.className='csl-item'+(isActive?' active':'');
    item.innerHTML=`<div class="csl-row"><span class="csl-num">${i+1}</span><span class="csl-name">${esc(m.nome)}</span>${m.tom?`<span class="csl-tom">${esc(m.tom)}</span>`:''}</div><div class="csl-expanded">${info?.artista?`<div class="csl-artist">${esc(info.artista)}</div>`:''}</div>`;
    item.addEventListener('click',()=>cSelectSong(i,m,info));
    el.appendChild(item);
  });
  // mostra player bar se há algo tocando
...
function cSelectSong(idx,m,info) {
  ...
  cRenderSongList();
  // toca a música
  const url=resolveAudioUrl(m?.mp3);
  if (!url) return;
...
function cSelectMonth(idx) {
  if(idx===cMIdx)return;
  cMIdx=idx;cCIdx=0;cSIdx=0;
  document.querySelectorAll('.c-mpill').forEach((p,i)=>p.classList.toggle('active',i===idx));
  const strip=document.getElementById('c-months'),pill=strip.children[idx];
  if(pill)strip.scrollLeft=pill.offsetLeft-strip.offsetWidth/2+pill.offsetWidth/2;
  cRenderAll();
}
function cNavCulto(dir) {
  const cs=cMonths[cMIdx]?.cultos||[],n=cCIdx+dir;
  if(n<0||n>=cs.length)return;
  cCIdx=n;cSIdx=0;cRenderCarousel();cRenderInfo();cRenderSongList();
}
function cSelectCulto(idx) {
  if(idx===cCIdx)return;
  cCIdx=idx;cSIdx=0;cRenderCarousel();cRenderInfo();cRenderSongList();
}

// touch cultos
let cTx0=0,cTy0=0,cTZ=null;
document.addEventListener('touchstart',e=>{
  if(!document.getElementById('panel-cultos').classList.contains('active'))return;
  cTx0=e.touches[0].clientX;cTy0=e.touches[0].clientY;
  cTZ=e.target.closest('#c-stage,#c-info')?'culto':null;
},{passive:true});
document.addEventListener('touchend',e=>{
  if(!cTZ)return;
  const dx=e.changedTouches[0].clientX-cTx0,dy=e.changedTouches[0].clientY-cTy0;
  if(Math.abs(dx)<Math.abs(dy)||Math.abs(dx)<38){cTZ=null;return;}
  cNavCulto(dx<0?1:-1);cTZ=null;
},{passive:true});
...
function cRenderPendente() {
  const el=document.getElementById('c-pend-list');
  document.getElementById('c-pend-ct').textContent=cultoPendente.length?`(${cultoPendente.length})`:'';
  if(!cultoPendente.length){el.innerHTML='<div style="color:rgba(255,255,255,.22);font-size:12px;padding:4px 0;">Nenhuma música adicionada.</div>';return;}
  el.innerHTML=cultoPendente.map((m,i)=>`<div class="c-pend-item"><span class="c-pend-num">${i+1}.</span><span class="c-pend-name">${esc(m.nome)}</span><input class="c-pend-tom" placeholder="Tom" value="${esc(m.tom)}" oninput="cultoPendente[${i}].tom=this.value"><button class="c-pend-rm" onclick="cultoPendente.splice(${i},1);cRenderPendente()">×</button></div>`).join('');
}
function cAbrirTopMes() {
  const now=new Date(),top=cGetTopMes(now.getMonth(),now.getFullYear());
  document.getElementById('c-tm-title').textContent=C_MESES[now.getMonth()];
  document.getElementById('c-tm-sub').textContent=top.length?`${top.length} músicas tocadas`:'Nenhum culto este mês.';
  document.getElementById('c-tm-rows').innerHTML=top.slice(0,5).map((t,i)=>`<div class="c-tm-row"><span class="c-tm-rank">${i+1}</span><span class="c-tm-name">${esc(t.nome)}</span><span class="c-tm-cnt">${t.cnt}×</span></div>`).join('');
  document.getElementById('c-tm-ov').classList.add('open');
}
function cFecharTopMes() { document.getElementById('c-tm-ov').classList.remove('open'); }
function selecionarMusica(nome) {
  const m=musicas.find(x=>x.nome===nome); if(!m)return;
  if(!cultoPendente.find(x=>x.nome===nome)){cultoPendente.push({nome:m.nome,tom:''});cRenderPendente();}
  fecharPicker();
}
async function salvarCulto() {
  const data=document.getElementById('c-data').value.trim(),ministra=document.getElementById('c-ministra').value.trim();
  if(!data){toast('Data é obrigatória.',true);return;}
  if(!cultoPendente.length){toast('Adicione pelo menos uma música.',true);return;}
  const btn=document.getElementById('c-btn-save'); btn.disabled=true; btn.textContent='Salvando…';
  const ok=await chamarScript({acao:'adicionarCulto',data,ministra,musicas:cultoPendente});
  btn.disabled=false; btn.textContent='Salvar culto na planilha';
  if(ok){toast('Culto salvo!');document.getElementById('c-data').value='';document.getElementById('c-ministra').value='';cultoPendente=[];cFecharSheet();setTimeout(()=>location.reload(),1500);}
}
async function chamarScript(payload) {
  try{const r=await fetch(SCRIPT_URL,{method:'POST',body:JSON.stringify(payload)});const j=await r.json();if(j.status==='ok')return true;toast('Erro: '+j.msg,true);return false;}
  catch(e){toast('Erro de conexão.',true);return false;}
}
...
// ═══════════════════════════════════════
// ENSAIO — preservado do original
// ═══════════════════════════════════════
const HOLYRICS_URL='http://10.10.10.44:8060/view/standard';
const LOADING_MSGS=['Afinando as cordas…','Buscando cifras no Drive…','Preparando o setlist…','Quase lá…','Carregando partituras…','Chegando já…'];
let ensaioMusicas=[],ensaioIdx=0,ensaioWakeLock=null,loadingMsgInterval=null;
let touchStartX=0,touchStartY=0,ensaioPerfil=null;
...
function selecionarPerfil(perfil){
  ensaioPerfil=perfil;document.querySelectorAll('.ensaio-choice-card').forEach(c=>c.classList.remove('selected'));
  document.getElementById('choice-'+perfil).classList.add('selected');
  if(perfil==='vocalista'){abrirHolyrics();return;}
  document.getElementById('ensaio-instrumentista-area').style.display='block';renderSongList();
}
function renderSongList(){
  const infoEl=document.getElementById('ensaio-culto-info'),listEl=document.getElementById('ensaio-song-list');
  if(!cultos.length){infoEl.textContent='';listEl.innerHTML='<div class="ensaio-no-culto">Nenhum culto registrado ainda.</div>';return;}
  const c=cultos[0];
  infoEl.innerHTML='<strong>'+esc(c.data)+'</strong> &middot; '+c.musicas.length+' m&uacute;sica'+(c.musicas.length!==1?'s':'')+' &middot; '+(esc(c.ministra)||'&mdash;');
  ensaioMusicas=c.musicas.map(m=>{const info=musicas.find(x=>x.nome.toLowerCase()===m.nome.toLowerCase());const cStr=info?.cifra||'';return{nome:m.nome,artista:info?.artista||'',tom:m.tom||'',driveId:extrairDriveId(cStr),linksExternos:extrairLinksExternos(cStr)};});
  listEl.innerHTML=ensaioMusicas.map((m,i)=>`<button class="ensaio-song-btn" onclick="abrirCifra(${i})"><div class="ensaio-song-num">${i+1}.</div><div class="ensaio-song-info"><div class="ensaio-song-name">${esc(m.nome)}</div>${m.artista?`<div class="ensaio-song-artist">${esc(m.artista)}</div>`:''}</div>${m.tom?`<div class="ensaio-song-tom">${esc(m.tom)}</div>`:''}<div class="ensaio-song-arrow">&#x203a;</div></button>`).join('');
}
function abrirCifra(idx){ensaioIdx=idx;mostrarLoadingEnsaio();setTimeout(()=>{esconderLoadingEnsaio();abrirEnsaioFs();},1200);}
function abrirHolyrics(){document.getElementById('holyrics-iframe').src=HOLYRICS_URL;document.getElementById('holyrics-fs').style.display='block';document.body.style.overflow='hidden';if('wakeLock'in navigator)navigator.wakeLock.request('screen').then(wl=>ensaioWakeLock=wl).catch(()=>{});}
function fecharHolyrics(){document.getElementById('holyrics-fs').style.display='none';document.getElementById('holyrics-iframe').src='';document.body.style.overflow='';if(ensaioWakeLock){ensaioWakeLock.release().catch(()=>{});ensaioWakeLock=null;}document.querySelectorAll('.ensaio-choice-card').forEach(c=>c.classList.remove('selected'));ensaioPerfil=null;}
...
function mostrarLoadingEnsaio(){const el=document.getElementById('ensaio-loading-msg');let i=0;el.textContent=LOADING_MSGS[0];el.style.opacity='1';loadingMsgInterval=setInterval(()=>{el.style.opacity='0';setTimeout(()=>{i=(i+1)%LOADING_MSGS.length;el.textContent=LOADING_MSGS[i];el.style.opacity='1';},250);},900);document.getElementById('ensaio-loading').classList.add('open');}
function esconderLoadingEnsaio(){clearInterval(loadingMsgInterval);document.getElementById('ensaio-loading').classList.remove('open');}
async function abrirEnsaioFs(){
  document.getElementById('ensaio-fs').classList.add('open');document.getElementById('ensaio-bar').classList.add('open');document.body.style.overflow='hidden';
  renderEnsaioSlide();
  try{if('wakeLock'in navigator)ensaioWakeLock=await navigator.wakeLock.request('screen');}catch(e){}
  document.getElementById('ensaio-fs').addEventListener('touchstart',onEnsaioTouchStart,{passive:true});
  document.getElementById('ensaio-fs').addEventListener('touchend',onEnsaioTouchEnd,{passive:true});
}
function fecharEnsaio(){
  document.getElementById('ensaio-fs').classList.remove('open');document.getElementById('ensaio-bar').classList.remove('open');
  document.getElementById('ensaio-iframe').src='';document.getElementById('ensaio-no-cifra-msg').style.display='none';
  document.body.style.overflow='';fecharMenuEnsaio();
  if(ensaioWakeLock){ensaioWakeLock.release().catch(()=>{});ensaioWakeLock=null;}
  document.getElementById('ensaio-fs').removeEventListener('touchstart',onEnsaioTouchStart);
  document.getElementById('ensaio-fs').removeEventListener('touchend',onEnsaioTouchEnd);
}
function renderEnsaioSlide(){
  const m=ensaioMusicas[ensaioIdx],total=ensaioMusicas.length,iframe=document.getElementById('ensaio-iframe'),noMsg=document.getElementById('ensaio-no-cifra-msg');
  const nome=m.nome.length>28?m.nome.slice(0,26)+'…':m.nome;
  document.getElementById('ensaio-bar-nome').textContent=nome;
  document.getElementById('ensaio-prev').disabled=ensaioIdx===0;
  document.getElementById('ensaio-next').disabled=ensaioIdx===total-1;
  if(m.driveId){iframe.src='https://drive.google.com/file/d/'+m.driveId+'/preview?rm=minimal';iframe.style.display='block';noMsg.style.display='none';}
  else{iframe.src='';iframe.style.display='none';let html='<div class="icon">&#127925;</div><div>'+esc(m.nome)+'</div>';if(m.linksExternos.length)html+=m.linksExternos.map(u=>'<a href="'+esc(u)+'" class="ensaio-ext-link" target="_blank" rel="noopener">&#8599; Abrir cifra externa</a>').join('');else html+='<div style="font-size:13px;opacity:0.6;">Sem cifra cadastrada.</div>';noMsg.innerHTML=html;noMsg.style.display='flex';}
}
function navegarEnsaio(dir){const n=ensaioIdx+dir;if(n<0||n>=ensaioMusicas.length)return;ensaioIdx=n;renderEnsaioSlide();}
function onEnsaioTouchStart(e){touchStartX=e.touches[0].clientX;touchStartY=e.touches[0].clientY;}
function onEnsaioTouchEnd(e){if(e.touches.length>0)return;const dx=e.changedTouches[0].clientX-touchStartX,dy=e.changedTouches[0].clientY-touchStartY;if(Math.abs(dx)>Math.abs(dy)&&Math.abs(dx)>55)navegarEnsaio(dx<0?1:-1);}
...
function abrirMenuEnsaio(){
  document.getElementById('ensaio-menu-list').innerHTML=ensaioMusicas.map((m,i)=>`<div class="ensaio-menu-item ${i===ensaioIdx?'active':''}" onclick="irParaMusica(${i})"><div class="ensaio-menu-num">${i+1}</div><div class="ensaio-menu-info"><div class="ensaio-menu-nome">${esc(m.nome)}</div><div class="ensaio-menu-meta">${m.artista||''}${m.tom?' &middot; Tom '+esc(m.tom):''}</div></div>${i===ensaioIdx?'<div class="ensaio-menu-playing">&#9654;</div>':''}</div>`).join('');
  document.getElementById('ensaio-menu').classList.add('open');
}
...
function mostrarErroEnsaio(msg){esconderLoadingEnsaio();document.getElementById('ensaio-error-box').textContent=msg;document.getElementById('ensaio-error').classList.add('open');}
...
function renderPicker(){
  const q=document.getElementById('picker-search').value.toLowerCase(),el=document.getElementById('picker-list');
  el.innerHTML=musicas.filter(m=>m.nome.toLowerCase().includes(q)||m.artista.toLowerCase().includes(q)).slice(0,50).map(m=>`<div class="picker-item" onclick="selecionarMusica('${esc(m.nome)}')"><div><div class="picker-item-name">${esc(m.nome)}</div><div class="picker-item-artist">${esc(m.artista)}</div></div><span class="picker-item-badge">${m.usos}×</span></div>`).join('');
}
